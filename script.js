// Clash-Lite Colosseum v2 — improved elixir bar, troop placement, balanced troops, tower AI.
// Uses cards_pack.json for card definitions (not troops.json).
...
let CARD_DATA = [];
let deck = [], playerDeck = [], enemyDeck = [], playerHand = [], enemyHand = [];
let entities = [], towers = [];
let playerElixir = 4, enemyElixir = 4, MAX_ELIXIR = 10;
let ELIXIR_RATE = 0.45; // faster regen for snappier gameplay
...
// load cards
fetch('cards_pack.json').then(r=>r.json()).then(j=>{ CARD_DATA = j.cards || []; buildPool(); if(deck.length===0 && CARD_DATA.length>=8){ deck = CARD_DATA.slice(0,8).map(c=>c.id); renderDeckSlots(); } }).catch(e=>console.error(e));
...
function renderDeckSlots(){
  ...
  for(let i=0;i<8;i++){
    ...
    if(deck[i]){ slot.innerHTML = `<img src="icons/${deck[i]}.svg" style="width:48px;height:48px">`; slot.addEventListener('click', ()=> { deck.splice(i,1); renderDeckSlots(); }); }
    else { slot.textContent = '+'; slot.addEventListener('dragover', e=> e.preventDefault()); slot.addEventListener('drop', e=> { e.preventDefault(); deck[i] = e.dataTransfer.getData('text/plain'); renderDeckSlots(); }); }
    deckSlots.appendChild(slot);
  }
}

function addToDeck(id){ if(deck.length>=8){ alert('Deck full'); return; } deck.push(id); renderDeckSlots(); }
...
btnDeck.addEventListener('click', ()=> { alert('Deck Maker: drag cards into slots or click to add. Click a slot to remove.'); });

function startQuickMatch(){
  if(deck.length<4){ deck = shuffle(CARD_DATA.map(c=>c.id)).slice(0,8); renderDeckSlots(); }
  startMatch();
}
...
function startMatch(){
  ...
  playerElixir = 4; enemyElixir = 4; timer = 120; lastTime = performance.now(); gameRunning = true; entities = []; setupTowers(); requestAnimationFrame(loop);
}

function createAIDeck(){ const heavy = CARD_DATA.filter(c=> c.hp>=160).map(c=>c.id); const spells = CARD_DATA.filter(c=> c.type==='spell').map(c=>c.id); const ids = CARD_DATA.map(c=>c.id); let d = []; if(heavy.length) d.push(randomChoice(heavy)); if(spells.length) d.push(randomChoice(spells)); while(d.length<8) d.push(randomChoice(ids)); return shuffle(d); }
function drawCard(hand, deckList){ if(deckList.length) hand.push(deckList.shift()); }

function setupTowers(){
  towers = [
    {x:140,y:H*0.28,team:0,hp:220,maxHp:220,range:140,fireCd:0},
    {x:140,y:H*0.5,team:0,hp:480,maxHp:480,range:200,fireCd:0,main:true},
    {x:140,y:H*0.72,team:0,hp:220,maxHp:220,range:140,fireCd:0},
    {x:W-140,y:H*0.28,team:1,hp:220,maxHp:220,range:140,fireCd:0},
    {x:W-140,y:H*0.5,team:1,hp:480,maxHp:480,range:200,fireCd:0,main:true},
    {x:W-140,y:H*0.72,team:1,hp:220,maxHp:220,range:140,fireCd:0}
  ];
}

function renderHandUI(){ handArea.innerHTML=''; playerHand.forEach((id,i)=>{ const c = CARD_DATA.find(x=>x.id===id); const el = document.createElement('div'); el.className='handCard'; el.innerHTML = `<img src="icons/${id}.svg" style="width:64px;height:64px"><div style="font-size:13px">${c.name}</div><div style="font-weight:900">${c.cost}</div>`; el.addEventListener('click', ()=> selectedHandIndex = i); handArea.appendChild(el); }); elixirValue.innerText = Math.floor(playerElixir); elixirMax.innerText = MAX_ELIXIR; const pct = Math.min(1, playerElixir / MAX_ELIXIR); elixirFill.style.width = (pct*100) + '%'; elixirFill.style.boxShadow = `0 8px ${8 + pct*40}px rgba(255,184,90,${0.08 + pct*0.3})`; }

canvas.addEventListener('click', (e)=>{ if(selectedHandIndex===null) return; const rect = canvas.getBoundingClientRect(); const x = e.clientX - rect.left, y = e.clientY - rect.top; if(x > W*0.55) return; const id = playerHand[selectedHandIndex]; const card = CARD_DATA.find(c=>c.id===id); if(!card) return; if(playerElixir < card.cost){ alert('Not enough elixir'); return; } deployCardAt(id, 0, Math.max(100, Math.min(x, W*0.5 - 40)), Math.max(80, Math.min(y, H-80))); playerElixir -= card.cost; playerDeck.push(playerHand.splice(selectedHandIndex,1)[0]); selectedHandIndex = null; drawCard(playerHand, playerDeck); renderHandUI(); });

function deployCardAt(id, team, x, y){
  const def = CARD_DATA.find(c=>c.id===id); if(!def) return;
  if(def.type==='spell'){ entities.push({kind:'spell',id:id,team:team,x:x,y:y,radius:def.radius||60,atk:def.atk||60,ttl:0.9}); }
...
function aiBehavior(dt){
  aiTimer += dt;
  if(aiTimer > 0.8){ aiTimer = 0; for(let i=0;i<enemyHand.length;i++){ const id = enemyHand[i]; const def = CARD_DATA.find(c=>c.id===id); if(def && def.cost <= enemyElixir){ enemyElixir -= def.cost; const tx = W - 220 + (Math.random()-0.5)*40; const ty = (Math.random()<0.5? H*0.28 : H*0.72) + (Math.random()-0.5)*24; deployCardAt(id,1, Math.max(W*0.55, Math.min(tx, W-120)), ty); enemyDeck.push(enemyHand.splice(i,1)[0]); drawCard(enemyHand, enemyDeck); break; } } }
}

// Entities update: movement, combat, healing, tower attacks
function updateEntities(dt){
  // towers auto attack
  towers.forEach(t=>{ t.fireCd -= dt; if(t.fireCd < 0) t.fireCd = 0; if(t.hp>0){ const enemies = entities.filter(e=> e.kind==='unit' && e.team !== t.team); if(enemies.length){ enemies.sort((a,b)=> Math.hypot(a.x-t.x,a.y-t.y) - Math.hypot(b.x-t.x,b.y-t.y)); const target = enemies[0]; const d = Math.hypot(target.x-t.x,target.y-t.y); if(d <= t.range && t.fireCd===0){ // fire projectile
        t.fireCd = 1.1; // tower cooldown
        // projectile hits instantly for simplicity
        target.hp -= 36;
  ...
  for(let i=entities.length-1;i>=0;i--){
    ...
    if(e.kind==='unit'){
      ...
      // target nearest enemy unit else tower
...
function loop(ts){
  ...
  // elixir regen faster towards end
  ...
  else { gameRunning=false; const p=towers.filter(t=>t.team===0).length, e=towers.filter(t=>t.team===1).length; const winner = p>e? 'Player' : (e>p? 'Enemy' : 'Draw'); setTimeout(()=>{ alert('Match ended — winner: '+winner); menu.style.display='flex'; gameContainer.classList.add('hidden'); },200); }
}
...
function drawScene(){
  ...
  towers.forEach(t=>{
    ctx.save();
    ctx.fillStyle = t.team===0? '#072a2b' : '#3a0707';
  ...
  entities.forEach(e=>{
    if(e.kind==='unit'){
      ...
      const baseHp = CARD_DATA.find(c=>c.id===e.id).hp || e.hp;
      ctx.fillStyle='#00d46b'; ctx.fillRect(e.x - e.size -2, e.y + e.size + 8, ((e.hp / baseHp) * ((e.size*2)+4)), 6);
    } else if(e.kind==='spell'){
  ...
  // particles
  particles.forEach(p=>{ ctx.globalAlpha = Math.max(0, p.life/0.8); ctx.fillStyle = p.color; ctx.beginPath(); ctx.arc(p.x,p.y,2.6,0,Math.PI*2); ctx.fill(); ctx.globalAlpha=1; });

  // top HUD
  ctx.fillStyle='rgba(255,255,255,0.06)'; ctx.fillRect(0,0,W,48);
  ctx.fillStyle='#ffd86b'; ctx.font='18px Inter, sans-serif'; ctx.fillText('Clash‑Lite Colosseum v2', 18, 34);
...
function drawColosseumBackground(){
  ...
  grd.addColorStop(0, '#2b0b06'); grd.addColorStop(1, '#040305'); ctx.fillStyle = grd; ctx.fillRect(0,0,W,H);
  for(let i=0;i<6;i++){ const r = 380 - i*48; ctx.beginPath(); ctx.lineWidth = 18 - i*2; ctx.strokeStyle = (i%2===0)? '#2a0b06' : '#470d06'; ctx.arc(W*0.5, H*0.55, r, 0, Math.PI*2); ctx.stroke(); }
  for(let i=0;i<8;i++){ const ang=(i/8)*Math.PI*2; const bx=W*0.5 + Math.cos(ang)*380; const by=H*0.55 + Math.sin(ang)*380; ctx.save(); ctx.translate(bx,by); ctx.rotate(ang+Math.PI/2); ctx.fillStyle=(i%2===0)? '#ff3f2f' : '#ffd86b'; roundRect(ctx,-12,-40,24,60,6,true,false); ctx.restore(); }
...
// initial quick-testing: prefill hands and deck for convenience
setInterval(()=> renderHandUI(), 300);
setTimeout(()=>{ if(deck.length===0 && CARD_DATA.length>=8){ deck = CARD_DATA.slice(0,8).map(c=>c.id); renderDeckSlots(); } }, 500);

// expose deploy for quick testing in console
